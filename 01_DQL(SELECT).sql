...
-- EMPLOYEE 테이블의 사원명, 급여, 보너스, 연봉, 보너스포함 연봉 조회(급여 + (급여 * 보너스))
SELECT EMP_NAME, SALARY, BONUS, SALARY * 12, (SALARY + (SALARY * 12))
FROM employee;
...
/*
    ...
    조건A OR 조건B -> 조건A와 조건B중 하나라도 만족하는 값은 가지고온다.
...
/*
    ...
    EX) 비교할대상컬럼 LIKE '_문자' : 비교대상컬럼값 문자앞에 아무글자나 N글자가 있는 값을 조회
        비교할대상컬럼 LIKE '문자__' : 비교대상컬럼값 문자뒤에 아무글자나 N글자가 있는 값을 조회
        비교할대상컬럼 LIKE '___문자__' : 비교대상컬럼값 문자앞뒤에 아무글자나 N글자가 있는 값을 조회
*/
...
--사원들중에서 이름에 중간글자가 '하'인 사원의 이름 전화번호 목록을 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
...
--직급코드가 J7이거나 J2인 사원들 중에 급여가 200만원 이상인 사원들의 모든 컬럼조회
SELECT *
FROM EMPLOYEE
...
--4. 급여가 200만원 이상이고 500만원 이하인 사원 중 입사일이 '01/01/01'이상이고 보너스를 받지않는 사원들의
