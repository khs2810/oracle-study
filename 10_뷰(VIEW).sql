/*
    ...
    (자주 사용하는 SELCT문을 저장해두면 긴 SELCT문을 매번 다시 기술할 필요가 없다.)
...
/*
    *뷰 컬럼에 별칭부여
    서브쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어있을 경우 반드시 별칭을 지정해야 한다.
*/
...
--200번 사원의 급여를 200만원으로 변경(SALARY >= 3000000 조건에 맞지않는 변경)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
...
--200번 사원의 급여를 200만원으로 변경(SALARY >= 3000000 조건에 맞지않는 변경)이 되지 않는다.
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
