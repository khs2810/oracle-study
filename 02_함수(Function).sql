...
/*
    *TRIM
    ...
    TRIM([LEADING | TRAILING | BOTH] 제거하고자하는 문자열)
*/

SELECT TRIM('     K  H    ') FROM DUAL; -- 양쪽에 있는 공백을 제거
SELECT TRIM('Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL; -- 양쪽에 있는 특정문자 제거

SELECT TRIM(LEADING 'Z' FROM 'ZZZZKHZZZZZZZ')FROM DUAL; -- LTRIM유사한 기능
...
FROM EMPLOYEE
WHERE JOB_CODE in ('J7', 'J6') AND SALARY >= 2000000 AND
...
SELECT ADD_MONTHS(SYSDATE, 4) AS 마지막날 FROM DUAL;
...
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일"') AS "입사날짜" -- 정해진 형식대로만 사용가능
FROM EMPLOYEE;
...
SELECT HIRE_DATE, TO_CHAR(HIRE_DATE, 'YYYY'), TO_CHAR(HIRE_DATE, 'RR') FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY AS "인상전급여"
      , DECODE(JOB_CODE
      , 'J7', SALARY * 1.1
      , 'J6', SALARY * 1.15
      , 'J5', SALARY * 1.2
